...
class ExportController extends Controller
{
    ...
    public function exportExcel($formId)
    {
        try {
            ...
            return Excel::download(new ResponsesExport($data['form_questions'], collect($data['responses'])), 'test.xlsx');
        } catch (\Exception $e) {
            return $this->returnError(201, $e->getMessage());
        }
    }
    ...
    public function exportPdf($formId)
    {
        try {
            ...
            return view('pdf.pdf', compact('response', 'form_questions','formId'));
        } catch (\Exception $e) {
            return $this->returnError(201, $e->getMessage());
        }
    }

    public function covertHTMlToPdf($formId)
    {
        try {
            $data = $this->prepareData($formId);
            view()->share('pdf.pdf', $data['responses']);
            $pdf = PDF::loadView('pdf.pdf', $data['responses']);
